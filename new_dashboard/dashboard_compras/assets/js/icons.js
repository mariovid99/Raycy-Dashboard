/* Iconos SVG inline — Lucide paths */
...
const PATHS = {
  ...
  'arrow-down-right': `<path d="M7 7v10h10"/><path d="M7 17 17 7"/>`,
};

export function Icon(name, { size = 16, className = '', strokeWidth = 1.75 } = {}) {
  const paths = PATHS[name];
  if (!paths) return '';
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round" class="${className}" aria-hidden="true">${paths}</svg>`;
}
